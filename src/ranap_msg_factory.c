 ...
 *
 *  UMTS_AMR      RAB subflows    Total
 *  RFCI          1   2    3      bits    Source rate
 *
 *  2             42  53   0      95      AMR 4,75 kbps
 *  3             49  54   0      103     AMR 5,15 kbps
 *  4             55  63   0      118     AMR 5,9 kbps
 *  5             58  76   0      134     AMR 6,7 kbps
 *  6             61  87   0      148     AMR 7,4 kbps
 *  7             75  84   0      159     AMR 7,95 kbps
 *  8             65  99   40     204     AMR 10,2 kbps
 *  9             81  103  60     244     AMR 12,2 kbps
 *  1             39  0    0      39      AMR SID
...
static RANAP_SDU_ParameterItem_t *new_sdu_par_item(enum sdu_par_profile profile, enum osmo_ranap_rab_mode modes)
{
...
/* See Chapter 5 of TS 26.102 */
static RANAP_RAB_Parameters_t *new_rab_par_voice(uint16_t modes)
{
	...
	sdui = new_sdu_par_item(SDUPAR_P_VOICE0, modes);
	ASN_SEQUENCE_ADD(&rab->sDU_Parameters, sdui);
	sdui = new_sdu_par_item(SDUPAR_P_VOICE1, modes);
	ASN_SEQUENCE_ADD(&rab->sDU_Parameters, sdui);
 ...
 * The 'modes' argument passes a bitmask indicating which AMR modes should be present, and SDUs are added accordingly.
 ...
 * \param modes  Bitmask of AMR modes, indicating which SDUs to include in the generated message.
 */
					      ...
					      uint16_t modes)
{
	...
	first.rAB_Parameters = new_rab_par_voice(modes);
